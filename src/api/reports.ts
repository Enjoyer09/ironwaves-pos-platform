...
export const z_report = async (
  ...
  if (isBackendEnabled()) {
    ...
    return {
      ...
      expected_cash: String(res?.expected_cash || '0'),
      actual_cash: String(res?.actual_cash || actual_cash || '0'),
      difference: String(res?.difference || '0'),
      deposit_total: String(res?.deposit_total || '0'),
      open_deposit_liability: String(res?.open_deposit_liability || '0'),
      closing_deposit_liability: String(res?.closing_deposit_liability || '0'),
      deposit_settled_amount: String(res?.deposit_settled_amount || '0'),
      closing_cash: String(res?.closing_cash || res?.actual_cash || actual_cash || '0'),
      opening_cash: String(res?.opening_cash || '0'),
      cash_movements_in: String(res?.cash_movements_in || '0'),
      cash_movements_out: String(res?.cash_movements_out || '0'),
      other_income_total: String(res?.other_income_total || '0'),
      other_income_lines: Array.isArray(res?.other_income_lines) ? res.other_income_lines : [],
      other_expense_total: String(res?.other_expense_total || '0'),
      other_expense_lines: Array.isArray(res?.other_expense_lines) ? res.other_expense_lines : [],
      receipt_html,
      email_sent: false,
      email_error: '',
    };
