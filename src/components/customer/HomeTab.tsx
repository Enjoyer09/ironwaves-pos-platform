...
export default function HomeTab({
  ...
  const headerBtn   = isLight ? 'bg-black/5 border-black/8 text-slate-800' : 'bg-white/5 border-white/10 text-white/80';
  ...
  const borderSec   = isLight ? 'border-black/8' : 'border-white/10';
  ...
  const inputSearch = isLight ? 'bg-black/5 border-black/8 text-slate-900 placeholder-slate-400' : 'bg-white/5 border-white/10 text-white placeholder-white/40';
  const walletBtn   = isLight ? 'bg-slate-100 hover:bg-slate-200 text-slate-800 border-black/8' : 'bg-black/80 hover:bg-black/90 text-white border-white/10';
  const comboCard   = isLight ? 'bg-orange-50/50 border-orange-100/70 hover:bg-orange-50' : 'bg-gradient-to-r from-[#F48C24]/10 to-[#ffb366]/5 border-white/10 hover:border-white/20';
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between px-1 mb-4">
        <button type="button" onClick={() => setActiveTab('profile')}
          className={`h-10 w-10 rounded-full border flex items-center justify-center active:scale-95 transition ${headerBtn}`}>
          <Menu size={20} />
        </button>
        <button type="button" onClick={() => setActiveTab('profile')}
          className={`relative h-10 w-10 rounded-full border flex items-center justify-center font-black active:scale-95 transition ${headerBtn}`}>
      ...
      <div className="px-1 mb-6 flex gap-3 animate-fadeInUp animate-fadeInUp-delay-1">
        <div className="relative flex-1">
          <input type="text" readOnly
            ...
            className={`w-full rounded-full border px-10 py-3 text-xs transition duration-200 cursor-pointer focus:outline-none focus:ring-1 focus:ring-[#F48C24] ${inputSearch}`}
      ...
      <div onClick={async (e) => { spawnParticles(e); playTickSound(); setCardFlipped(!cardFlipped); await nativeHapticImpact(ImpactStyle.Light); }}
        ...
        <div className={`relative w-full h-full duration-700 preserve-3d transition-transform ${cardFlipped ? 'rotate-y-180' : ''}`}>
          {/* CARD FRONT */}
          <div className="absolute inset-0 backface-hidden border p-6 flex flex-col justify-between overflow-hidden shadow-[0_16px_36px_rgba(26,67,41,0.15)]"
          ...
          <div>
            <div className="flex items-end justify-between mb-4">
              <div>
                ...
                <p className={`mt-1 text-3xl font-black tracking-tight animate-countReveal ${headerText}`}>
                  <AnimatedCounter value={Number(wallet.stars_balance ?? 0)} decimals={programMode === 'cashback' ? 2 : 0} suffix={balanceSuffix} />
                </p>
              </div>
              <div className="text-right">
                <span className={`inline-block rounded-full px-2.5 py-1 text-[10px] font-bold tracking-wider uppercase border ${isLight ? 'bg-black/5 border-black/8 text-slate-800' : 'bg-white/5 border-white/10 text-white'}`}>
            ...
            {programMode === 'points' ? (
                ...
                <div className="flex items-center gap-5">
                  <div className="relative select-none flex-shrink-0 flex items-center justify-center">
                    <svg viewBox="0 0 100 110" className="w-16 h-18 overflow-visible">
                      ...
                      <path d="M76 35 C90 35, 90 65, 76 65" fill="none" stroke={isLight ? "rgba(0,0,0,0.1)" : "rgba(255,255,255,0.15)"} strokeWidth="4.5" strokeLinecap="round" />
                      <path d="M20 18 L80 18 L73 85 C72 93, 28 93, 27 85 Z" fill={isLight ? "rgba(0,0,0,0.02)" : "rgba(255,255,255,0.02)"} stroke={isLight ? "rgba(0,0,0,0.12)" : "rgba(255,255,255,0.2)"} strokeWidth="2.5" />
                      ...
                      {[
                        ...
                        return (
                          <div key={mIdx} className="flex items-center gap-2 text-[10px]">
                            <span className={`h-2 w-2 rounded-full ${isUnlocked ? 'bg-[#F48C24] shadow-[0_0_6px_rgba(244,140,36,0.8)]' : isLight ? 'bg-black/10' : 'bg-white/10'}`} />
              ...
              <div className={`border-t pt-3 ${borderSec}`}>
                <div className={`h-1.5 overflow-hidden rounded-full ${isLight ? 'bg-black/10' : 'bg-white/10'}`}>
            ...
            <div className={`pt-3 mt-3 border-t flex flex-row gap-2 justify-center items-center ${borderSec}`}>
              <a href={get_customer_wallet_pass_url_fn(sessionCreds.cardId, sessionCreds.token, safeLang)} target="_blank" rel="noopener noreferrer"
                ...
                className={`flex-1 flex items-center justify-center gap-1.5 rounded-xl py-2 border transition text-[10px] font-semibold active:scale-95 ${walletBtn}`}>
      ...
      <div className="grid grid-cols-2 gap-3.5 animate-fadeInUp animate-fadeInUp-delay-3">
        <section className={`rounded-[28px] p-5 flex flex-col justify-between border shadow-sm ${bgCard}`}>
          <div>
        ...
        <section onClick={async () => { setCardFlipped(!cardFlipped); playTickSound(); await nativeHapticImpact(ImpactStyle.Medium); }}
          className={`rounded-[28px] p-5 flex flex-col justify-between border shadow-sm transition active:scale-[0.98] cursor-pointer ${bgCard}`}>
          <div className={`flex items-center gap-2 text-[11px] font-bold uppercase tracking-wider ${subText}`}>
          ...
          {showQrCard && cardQr ? (
            <div className="mt-3 flex flex-col items-center">
              <div className="p-2 bg-white rounded-2xl border border-black/5 shadow-inner">
      ...
      <section className={`rounded-[28px] p-5 border shadow-sm space-y-4 animate-fadeInUp animate-fadeInUp-delay-4 ${bgCard}`}>
        <div className="flex items-center justify-between">
          ...
          <button type="button" onClick={async () => { await nativeHapticImpact(ImpactStyle.Light); setSimulatedTemp(t => t > 20 ? 14 : 26); setSimulatedCondition(c => c === 'sunny' ? 'rainy' : 'sunny'); }}
            className={`rounded-full border px-2.5 py-1 text-[9px] font-bold active:scale-95 transition ${isLight ? 'bg-black/5 border-black/8 text-slate-700 hover:bg-black/8' : 'bg-white/5 border-white/10 text-white hover:bg-white/10'}`}>
          ...
          <div className="grid grid-cols-2 gap-2">
            {getWeatherInfo(safeLang, simulatedTemp).recommendedDrinks.map((drink, idx) => (
              <div key={idx} onClick={async () => { setCardFlipped(!cardFlipped); playTickSound(); await nativeHapticImpact(ImpactStyle.Light); }}
                className={`flex items-center gap-2.5 p-3 rounded-2xl border transition cursor-pointer active:scale-95 ${isLight ? 'bg-black/3 border-black/5 hover:bg-black/5' : 'bg-white/5 border-white/5 hover:bg-white/10 hover:border-white/10'}`}>
        ...
        <div className={`pt-3 border-t ${borderSec}`}>
          ...
          {getWeatherInfo(safeLang, simulatedTemp).comboItems.map((combo, idx) => (
            <div key={idx} onClick={async () => { setCardFlipped(!cardFlipped); playTickSound(); await nativeHapticImpact(ImpactStyle.Light); }}
              className={`flex items-center gap-3 p-3 rounded-2xl border transition cursor-pointer active:scale-[0.98] ${comboCard}`}>
      ...
      {favoriteItems.length > 0 && (
        <section className={`rounded-[28px] p-5 border shadow-sm space-y-4 animate-fadeInUp animate-fadeInUp-delay-5 ${bgCard}`}>
          ...
          <div className="flex gap-3 overflow-x-auto pb-1">
            {favoriteItems.map((item: any) => (
              <div key={item.name} onClick={async () => { setCardFlipped(!cardFlipped); playTickSound(); await nativeHapticImpact(ImpactStyle.Light); }}
                className={`flex items-center gap-3 shrink-0 rounded-2xl p-3 border active:scale-95 transition-transform cursor-pointer ${isLight ? 'bg-black/3 border-black/5 hover:border-black/10' : 'bg-white/5 border-white/5 hover:border-white/10 text-white'}`}
                style={{ minWidth: '160px' }}>
                <div className={`h-10 w-10 rounded-xl flex items-center justify-center text-lg border ${isLight ? 'bg-black/5 border-black/5' : 'bg-white/10 border-white/5'}`}>
      ...
      {/* Active Codes */}
      <section className={`rounded-[28px] p-5 border shadow-sm animate-fadeInUp animate-fadeInUp-delay-5 ${bgCard}`}>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
        ...
        <div className="mt-4 flex gap-3.5 overflow-x-auto pb-1.5">
          {pendingClaims.length === 0 ? (
            <div className={`w-full rounded-2xl py-6 text-center text-xs border border-dashed ${isLight ? 'border-black/10 bg-black/3' : 'border-white/10 bg-white/5'} ${textMuted}`}>
            ...
            <div key={row.id} className={`relative min-w-[170px] shrink-0 rounded-2xl p-4 overflow-hidden border border-[#F48C24]/30 shadow-inner flex flex-col justify-between`}
              ...
              <div className={`absolute -left-1.5 top-1/2 -translate-y-1/2 w-3 h-3 rounded-full border-r border-[#F48C24]/20 ${isLight ? 'bg-white' : 'bg-[#0E0C0B]'}`} />
              <div className={`absolute -right-1.5 top-1/2 -translate-y-1/2 w-3 h-3 rounded-full border-l border-[#F48C24]/20 ${isLight ? 'bg-white' : 'bg-[#0E0C0B]'}`} />
              <div>
