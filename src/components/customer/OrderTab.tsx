...
export function ModifierSheet({
  ...
  const sheetBg     = isLight
    ...
    : 'bg-[#0D0B0A]/92 backdrop-blur-2xl';
  const sheetBorder = isLight ? 'border-black/8' : 'border-white/12';
  const textPrimary = isLight ? 'text-slate-900' : 'text-white';
  const textSecond  = isLight ? 'text-slate-500' : 'text-white/50';
  const chipBase    = isLight ? 'bg-black/5 border-black/10 text-slate-700 hover:bg-black/8' : 'bg-white/6 border-white/10 text-white/70 hover:bg-white/12';
  const btnBase     = isLight ? 'bg-black/5 hover:bg-black/10 border-black/8 text-slate-800' : 'bg-white/6 hover:bg-white/12 border-white/10 text-white';
  const divider     = isLight ? 'border-black/6' : 'border-white/8';
        ...
        <div className="flex-1 overflow-y-auto px-5 pb-5 space-y-4 relative z-10">
          ...
          <div className="flex gap-4 items-start">
            ...
            <div className="flex-1 min-w-0 pt-0.5">
              ...
              <p className="text-xl font-black text-[#F48C24] mt-1.5 gradient-text-animated">
                ...
                {modifierSheetItem.variants.map((v: any) => {
                  ...
                  return (
                    <button key={v.name} type="button" onClick={() => setSelectedVariant(active ? null : v)}
                      className={`rounded-full px-3 py-1.5 text-[10px] font-bold border transition-all active:scale-95 ${active
                        ? 'bg-[#F48C24] border-[#F48C24] text-white glow-orange-sm shadow-lg scale-105'
                        : chipBase}`}>
                ...
                {modifierSheetItem.modifiers.map((mod: any) => {
                  ...
                  return (
                    <button key={mod.name} type="button" onClick={() => handleToggleModifier(mod)}
                      className={`rounded-full px-2.5 py-1.5 text-[9px] font-bold border transition-all active:scale-95 ${active
                        ? 'bg-[#F48C24] border-[#F48C24] text-white glow-orange-sm'
                        : chipBase}`}>
          ...
          <button type="button"
            ...
            className="w-full rounded-2xl py-4 text-sm font-black text-white transition-all active:scale-[0.97] shimmer-btn"
            style={{
...
export function CartSheet({
  ...
  const sheetBg     = isLight ? 'bg-white/92 backdrop-blur-2xl' : 'bg-[#0D0B0A]/92 backdrop-blur-2xl';
  const sheetBorder = isLight ? 'border-black/8' : 'border-white/12';
  const textPrimary = isLight ? 'text-slate-900' : 'text-white';
  const textSecond  = isLight ? 'text-slate-500' : 'text-white/50';
  const itemBg      = isLight ? 'bg-white border-black/6 shadow-sm' : 'bg-white/5 border-white/6';
  const divider     = isLight ? 'border-black/6' : 'border-white/8';
  ...
  return ReactDOM.createPortal(
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4"
      ...
      <div className={`relative w-full max-w-sm rounded-[32px] ${sheetBg} border ${sheetBorder} max-h-[88vh] p-5 flex flex-col`}
        ...
        {/* Cart Items */}
        <div className="space-y-2 flex-1 overflow-y-auto pr-1 relative z-10">
          {customerCart.length === 0 ? (
            ...
            <div key={idx} className={`flex items-center justify-between rounded-2xl ${itemBg} border p-3 transition-all`}>
              ...
              <div className="flex items-center gap-2 ml-2">
                <span className={`text-[10px] font-bold ${textSecond}`}>×{item.quantity}</span>
                ...
                <button onClick={() => handleRemoveFromCart(idx)}
                  className="h-6 w-6 rounded-full bg-red-500/12 text-red-500 flex items-center justify-center text-[10px] font-bold hover:bg-red-500/20 transition-all active:scale-90">
                  <X size={10} />
                </button>
              </div>
            </div>
          ...
          <div className="mt-4 flex flex-col gap-3 relative z-10">
            ...
            <div className={`pt-3 border-t ${divider}`}>
              ...
              <button
                ...
                className="w-full rounded-2xl text-white py-4 text-sm font-black transition active:scale-95 flex items-center justify-center gap-2 disabled:opacity-50 shimmer-btn"
                style={{
...
export default function OrderTab({
  ...
  const catInactive = isLight
    ? 'bg-white/80 border-black/8 text-slate-700 hover:bg-white shadow-sm backdrop-blur-sm'
    : 'bg-white/6 border-white/10 text-white/70 hover:bg-white/12 backdrop-blur-sm';
            ...
            return (
              <button key={cat} onClick={() => setSelectedCategory(cat)}
                className={`flex-none w-[76px] flex flex-col items-center gap-1.5 rounded-[22px] p-2.5 transition-all border ${
                  isSelected
                    ? 'text-white shimmer-btn glow-orange'
                    : catInactive
                }`}
                style={isSelected ? {
                  background: 'linear-gradient(135deg, #F48C24, #ffb366)',
                  borderColor: 'rgba(244,140,36,0.4)',
                } : undefined}>
                <div className={`h-11 w-11 rounded-full overflow-hidden border-2 shadow-sm ${isSelected ? 'border-white/40' : isLight ? 'border-black/8' : 'border-white/6'}`}>
