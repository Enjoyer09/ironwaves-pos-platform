...
export default function FalciTab({
  ...
  fileRef, analyzeImageFortune, takePhotoWithCamera, primaryColor, accentColor, isLight = false
}: Props) {
  ...
  return (
    <section className={`rounded-[28px] border p-5 space-y-4 ${bgCard}`}>
      ...
      <div className="flex items-center gap-2">
        ...
        <div>
          <div className={`text-md font-black tracking-tight ${textPrimary}`}>AI Falçı</div>
          <div className={`text-[11px] font-semibold ${textSecond}`}>{tx(safeLang, 'Bir şəkil yüklə, AI Falçı onun tonuna və ab-havasına baxıb əyləncəli mesaj versin.', 'Загрузи фото, и AI Falçı даст тебе игровое предсказание по атмосфере изображения.', 'Upload an image and AI Fortune Teller will give you a playful reading based on its vibe.')}</div>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-3">
        ...
        {Capacitor.isNativePlatform() && (
          <button
            ...
            className="flex items-center gap-2 rounded-2xl px-5 py-3 font-black text-[12px] text-white active:scale-95 transition-transform animate-pulse"
            style={{
      ...
      {fortuneImage && (
        <div className="relative rounded-[24px] overflow-hidden border border-black/10 shadow-2xl">
      ...
      {fortuneLoading ? (
        <div className={`flex flex-col items-center justify-center py-6 space-y-5 rounded-[24px] p-5 border ${loaderBg}`}>
          <div className="relative h-32 w-32 flex items-center justify-center animate-floatBall">
            ...
            <div className={`relative h-28 w-28 rounded-full border bg-white/5 backdrop-blur-md shadow-[0_0_40px_rgba(168,85,247,0.35),_inset_0_4px_16px_rgba(255,255,255,0.2)] flex flex-col items-center justify-center overflow-hidden ${isLight ? 'border-black/5' : 'border-white/20'}`}>
              <div className="absolute inset-2 rounded-full border border-dashed border-white/10 animate-ping opacity-20" />
              <span className={`text-2xl font-black ${isLight ? 'text-slate-800' : 'text-white'}`}>{fortuneProgress}%</span>
              <span className="text-[8px] font-black tracking-widest text-amber-500 uppercase mt-1">
                {tx(safeLang, 'TƏHLİL', 'АНАЛИЗ', 'ANALYSIS')}
              </span>
            </div>
          </div>
          <div className="text-center space-y-2.5 max-w-xs">
            <p className={`text-[12px] font-bold tracking-wide animate-pulse ${textPrimary}`}>
              {fortuneStepText}
            </p>
            <div className={`w-48 h-1 rounded-full mx-auto overflow-hidden ${isLight ? 'bg-black/10' : 'bg-white/10'}`}>
              <div
        ...
        <div className={`rounded-[24px] border p-5 text-[13px] font-medium leading-relaxed ${innerBoxBg}`}>
          {fortuneText ? (
            <div className="space-y-2">
              ...
              <p className={`italic ${isLight ? 'text-slate-700' : 'text-slate-100'}`}>{fortuneText}</p>
            </div>
          ) : (
            <p className={`text-center py-3 ${textMuted}`}>
