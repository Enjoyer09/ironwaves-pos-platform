...
def _run_startup_migrations():
    with engine.begin() as conn:
        ...
        conn.execute(text("ALTER TABLE sales ADD COLUMN IF NOT EXISTS campaign_id VARCHAR(36)"))
        conn.execute(text("ALTER TABLE business_profiles ADD COLUMN IF NOT EXISTS voen VARCHAR(64)"))
        conn.execute(text("ALTER TABLE business_profiles ADD COLUMN IF NOT EXISTS tax_regime VARCHAR(32) DEFAULT 'simplified'"))
        conn.execute(text("ALTER TABLE business_profiles ADD COLUMN IF NOT EXISTS vat_rate NUMERIC(5,2) DEFAULT 18"))
        conn.execute(text("ALTER TABLE business_profiles ADD COLUMN IF NOT EXISTS nka_registration_no VARCHAR(64)"))
        conn.execute(text("ALTER TABLE business_profiles ADD COLUMN IF NOT EXISTS fiscal_enabled BOOLEAN DEFAULT FALSE"))
