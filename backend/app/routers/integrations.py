...
async def bolt_webhook(
    ...
    for item in raw_items:
        ...
        if menu_item:
            ...
            mapped_name = f"Bolt Məhsulu ({item_name})" if item_name else f"Bolt Məhsulu ({ext_id})"
    ...
    post_sale_payment(
        ...
        amount=total,
        payment_source="card",
        created_by="Bolt Food Integration",
        category="Satış (Kart)",
        note=f"Bolt Food Order {order_id}",
    )
    ...
    post_sale_cogs(
        ...
        created_by="Bolt Food Integration",
        note=f"Bolt Food order COGS {sale.id}",
    )
    ...
    kitchen_order = KitchenOrder(
        tenant_id=tenant_id,
        sale_id=sale.id,
        table_label=f"Bolt #{order_id[-5:]}" if len(order_id) >= 5 else f"Bolt #{order_id}",
        order_type="Delivery",
        status="NEW",
        priority="NORMAL",
        items_json=json.dumps(cart_items, ensure_ascii=False),
    ...
    for username in usernames:
        db.add(
            StaffNotification(
                tenant_id=tenant_id,
                username=username,
                title="Yeni Bolt Food Sifarişi",
                message=f"Bolt sifarişi #{order_id[-5:] if len(order_id) >= 5 else order_id} mətbəxə ötürüldü: {summarized_items_text}",
...
async def wolt_webhook(
    ...
    for item in raw_items:
        ...
        if menu_item:
            ...
            mapped_name = f"Wolt Məhsulu ({item_name})" if item_name else f"Wolt Məhsulu ({ext_id})"
            mapped_category = "Delivery"
            is_coffee = False
            unit_price = price_val
    ...
    post_sale_payment(
        ...
        amount=total,
        payment_source="card",
        created_by="Wolt Integration",
        category="Satış (Kart)",
        note=f"Wolt Order {order_id}",
    )
    ...
    post_sale_cogs(
        ...
        amount=Decimal(str(cogs_total or 0)).quantize(Decimal("0.01")),
        created_by="Wolt Integration",
        note=f"Wolt order COGS {sale.id}",
    )
    ...
    kitchen_order = KitchenOrder(
        tenant_id=tenant_id,
        sale_id=sale.id,
        table_label=f"Wolt #{order_id[-5:]}" if len(order_id) >= 5 else f"Wolt #{order_id}",
        order_type="Delivery",
        status="NEW",
        priority="NORMAL",
        items_json=json.dumps(cart_items, ensure_ascii=False),
    ...
    for username in usernames:
        db.add(
            StaffNotification(
                tenant_id=tenant_id,
                username=username,
                title="Yeni Wolt Sifarişi",
                message=f"Wolt sifarişi #{order_id[-5:] if len(order_id) >= 5 else order_id} mətbəxə ötürüldü: {summarized_items_text}",
                meta_json=json.dumps({"sale_id": sale.id, "kitchen_order_id": kitchen_order.id}, ensure_ascii=False),
                is_read=False,
            )
        )
