...
class ManualRestoreIn(BaseModel):
    cash_target: Decimal
    card_target: Decimal

@router.post("/manual-restore")
def manual_restore(
    payload: ManualRestoreIn,
    db: Session = Depends(get_db),
    tenant: Tenant = Depends(get_tenant),
    user=Depends(get_current_user),
):
    _ensure_finance_write_access(user)
    if tenant.domain != "emalatcoffee.ironwaves.store" and tenant.slug != "emalatcoffee":
        raise HTTPException(status_code=403, detail="Yalnız emalatcoffee tenantı üçün icazəlidir")
    curr_cash = _wallet_balance(db, tenant.id, "cash")
    curr_card = _wallet_balance(db, tenant.id, "card")
    cash_diff = Decimal(str(payload.cash_target)) - curr_cash
    card_diff = Decimal(str(payload.card_target)) - curr_card
    ...
    if not actions:
        return {"success": True, "message": "Balanslar onsuz da hədəf məbləğlərlə eynidir"}
    ...
    db.add(
        AuditLog(
            tenant_id=tenant.id,
            user=user.username,
            action="FINANCE_MANUAL_CASH_RESTORE_UI",
            details=json.dumps({
                "cash_before": str(curr_cash),
                "cash_after": str(payload.cash_target),
                "card_before": str(curr_card),
                "card_after": str(payload.card_target),
            }, ensure_ascii=False)
        )
    )
    db.commit()
    return {"success": True, "message": "Balanslar uğurla yeniləndi"}
